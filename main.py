...
async def tmdb_get(path: str, params: Dict[str, Any], _retries: int = 3) -> Dict[str, Any]:
    """
    Safe TMDB GET with retry + backoff:
    ...
    for attempt in range(_retries):
        try:
            ...
            raise  # don't retry HTTP-level errors
    ...
    raise HTTPException(
        status_code=502,
        detail=f"TMDB unreachable after {_retries} attempts: {type(last_exc).__name__}",
    )
...
async def home(
    ...
    genre_id: optional TMDB genre ID to filter results via /discover/movie
    """
    try:
